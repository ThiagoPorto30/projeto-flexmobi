...
export default function ProductView({ bike, details, related }: ProductViewProps) {
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.mobileMenuShell}><div id="product-mobile-navigation" className={`${styles.mobileMenu} t-panel-slide`} data-open={mobileMenuOpen} aria-hidden={!mobileMenuOpen} inert={!mobileMenuOpen}><nav aria-label="Navegação móvel do produto"><Link href="/#catalogo" onClick={() => setMobileMenuOpen(false)}>Modelos</Link><a href="#desempenho" onClick={() => setMobileMenuOpen(false)}>Destaques</a><a href="#ficha" onClick={() => setMobileMenuOpen(false)}>Ficha técnica</a><Link href="/#lojas" onClick={() => setMobileMenuOpen(false)}>Lojas</Link></nav><Link className={styles.mobileMenuCta} href={testRideHref}>Agendar test ride <Arrow /></Link></div></div>
