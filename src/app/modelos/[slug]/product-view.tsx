...
export default function ProductView({ bike, details, related }: ProductViewProps) {
  ...
  function showImage(index: number) {
    const boundedIndex = Math.max(0, Math.min(details.gallery.length - 1, index));
    if (boundedIndex === activeImage) return;
    setGalleryDirection(boundedIndex > activeImage ? "next" : "previous");
    setActiveImage(boundedIndex);
  }
  ...
  function moveGalleryDrag(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const isPastStart = activeImage === 0 && deltaX > 0;
    const isPastEnd = activeImage === details.gallery.length - 1 && deltaX < 0;
    const resistance = isPastStart || isPastEnd ? 0.22 : 0.72;
    const offset = Math.max(-78, Math.min(78, deltaX * resistance));
    event.currentTarget.style.setProperty("--gallery-drag-x", `${offset}px`);
  }

  function finishGalleryDrag(event: ReactPointerEvent<HTMLDivElement>, commit: boolean) {
    const start = dragStart.current;
    if (!start || start.pointerId !== event.pointerId) return;
    ...
    event.currentTarget.style.removeProperty("--gallery-drag-x");
    if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);

    if (!commit || Math.abs(deltaX) < 46 || Math.abs(deltaX) <= Math.abs(deltaY) * 1.15) return;
    showImage(activeImage + (deltaX < 0 ? 1 : -1));
  }

  function navigateGalleryWithKeyboard(event: ReactKeyboardEvent<HTMLDivElement>) {
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.mobileMenuShell}><div id="product-mobile-navigation" className={`${styles.mobileMenu} t-panel-slide`} data-open={mobileMenuOpen} aria-hidden={!mobileMenuOpen} inert={!mobileMenuOpen}><nav aria-label="Navegação móvel do produto"><Link href="/modelos" onClick={() => setMobileMenuOpen(false)}>Modelos</Link><a href="#desempenho" onClick={() => setMobileMenuOpen(false)}>Destaques</a><a href="#ficha" onClick={() => setMobileMenuOpen(false)}>Ficha técnica</a><Link href="/#lojas" onClick={() => setMobileMenuOpen(false)}>Lojas</Link></nav><Link className={styles.mobileMenuCta} href={testRideHref}>Agendar test ride</Link></div></div>

      <section className={styles.hero}>
        <nav className={styles.breadcrumbs} aria-label="Trilha de navegação">
          <Link href="/">Início</Link><span aria-hidden="true">/</span><Link href="/modelos">Modelos</Link><span aria-hidden="true">/</span><b aria-current="page">{bike.name.replace("INOW ", "")}</b>
        </nav>

        <div className={styles.gallery}>
          <div className={styles.galleryMeta}>
            <span aria-live="polite">{String(activeImage + 1).padStart(2, "0")} / {String(details.gallery.length).padStart(2, "0")}</span>
          ...
          <div
            ...
            role="group"
            tabIndex={0}
            aria-label={`Galeria de ${bike.name}. Use as setas ou arraste para mudar de imagem.`}
            onKeyDown={navigateGalleryWithKeyboard}
            onPointerDown={startGalleryDrag}
            onPointerMove={moveGalleryDrag}
            onPointerUp={(event) => finishGalleryDrag(event, true)}
            onPointerCancel={(event) => finishGalleryDrag(event, false)}
          >
        ...
        <div className={styles.productIntro}>
          ...
          <div className={styles.actions}>
            <Link className={styles.primaryButton} href={testRideHref}>
              {bike.available ? "Testar esta bike" : "Quero saber quando chegar"}
            </Link>
            <Link className={styles.secondaryButton} href="/#lojas">Ver as lojas</Link>
          </div>
      ...
      <section className={styles.closing}>
        ...
        <Link href={testRideHref}>Agendar test ride com a {bike.name.replace("INOW ", "")}</Link>
        <span>Icaraí / Niterói — Ipanema / Rio de Janeiro</span>
      </section>

      <SiteFooter />

      <Link className={styles.mobileCta} href={testRideHref}>Agendar test ride</Link>
    </main>
  );
}
